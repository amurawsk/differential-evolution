...
def main():
    dimensions = [10]
    bounds = (-100, 100)
    columns = ['function', 'dim', 'F', 'CR', 'mode', 'avg_fitnesses', 'result_vars', 'iteration']
            ...
            for dim in dimensions:
                for F in tqdm(range(1, 10)):
                    for CR in range(1, 10):
                        get_data(results_df=results_df, function=function, dim=dim, bounds=bounds, F=F/10, CR=CR/10, index=index, i=i, pop_size=int(10*dim), max_iter=int(dim*100))
